...
export const ranks = [
  ...
  ['captain', 50], ['commander', 100], ['admiral', 250],
];
...
function readCareer() {
  ...
  try {
    ...
    if (saved && validCount(saved.kills) && validCount(saved.bestRun)
      && saved.bestRun <= saved.kills) return saved;
  } catch {
    // Invalid or unavailable storage must not prevent flight.
  }
  return { kills: legacyKills, bestRun: legacyKills };
}
...
export function getCareer() {
  const index = ranks.findLastIndex(([, kills]) => career.kills >= kills);
  const [id, kills] = ranks[index];
  const next = ranks[index + 1];
  return {
    kills: career.kills, bestRun: career.bestRun, rank: { id, kills },
    nextRank: next ? { id: next[0], kills: next[1], remaining: next[1] - career.kills } : null,
